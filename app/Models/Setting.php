...
class Setting extends BaseModel implements HasMedia
{
    ...
    {
        // Query directly from database instead of cached collection to avoid stale data
        if ($setting = self::where('name', $key)->first()) {
    ...
    {
        // Query directly from database to avoid cache issues during updates
        return (bool) self::where('name', $key)->count();
    }
