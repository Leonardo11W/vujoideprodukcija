...
class BranchResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $holidayDaysInWeek = $this->holidays->filter(function ($holiday) use ($startOfWeek, $endOfWeek) {
            ...
            return Carbon::parse($holiday->date)->dayOfWeek; // 0 = Sun, 6 = Sat
        })->unique()->values(); // Clean duplicate days


        $workingDays = $this->businessHours->map(function ($hour) use ($holidayDaysInWeek) {
            // Normalize to Carbon's day format if needed
            ...
            return [
                ...
                'is_holiday' => ($hour['is_holiday'] == 0 && $holidayDaysInWeek->contains($dayAsInt)) ? 1 : $hour['is_holiday'],
                'breaks' => $hour['breaks'],
            ];
        });
        ...
        return [
            ...
            'city' => optional($this->address?->city_data)->name,
            'state' => optional($this->address?->state_data)->name,
            'country' => optional($this->address?->country_data)->name,
            ...
            'branch_image' => $this->media->pluck('original_url')->first() ?? asset('img/frontend/branch-image.jpg'),
