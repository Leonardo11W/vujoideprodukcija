...
class CommonNotification extends Notification implements ShouldQueue
{
    ...
    public function __construct($type, $data)
    {
        ...
        // Backward-compatibility for older templates that used [check-out time]
        if (isset($this->data['check_out_time'])) {
            $templateDetail = str_replace('[check-out time]', (string) $this->data['check_out_time'], $templateDetail);
        }
        $this->data['type'] = $templateData->notification_subject ?? 'None';
        $this->data['message'] = $templateDetail ?? __('messages.default_notification_body');
        $this->appData = $notifications->channels;
        $this->notification = $notifications;
        $this->mailable = $templateData;
