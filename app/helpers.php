    ...
    if ($mailable != null && $mailable->to != null) {
        ...
        foreach ($mails as $key => $mailTo) {
            ...
            if (isset($booking) && $booking != null) {
                ...
                $data['check_out_time'] = $booking['check_out_time'] ?? '';
...
function fcm($fields)
{
    ...
    if (!$accessToken) {
        \Log::info('FCM skipped: No credentials found.');
        return;
    }
...
/**
 * Store multiple media files to a collection without clearing existing ones
 *
 * @param mixed $module The model instance
 * @param array $files Array of uploaded files
 * @param string $collectionName Name of the media collection
 * @param bool $clearExisting Whether to clear existing files in the collection
 * @return void
 */
function storeMultipleMediaFiles($module, $files, $collectionName = 'gallery', $clearExisting = false)
{
    if (!isset($module) || empty($files)) {
        return;
    }

    // Clear existing files if needed
    if ($clearExisting) {
        $module->clearMediaCollection($collectionName);
    }

    // Process each file
    foreach ($files as $file) {
        if ($file && $file->isValid()) {
            $module->addMedia($file)->toMediaCollection($collectionName);
        }
    }
}
...
function isActiveProductDiscount($product)
{
    if (!$product) {
        return false;
    }
    ...
    // If both dates are null, we treat it as a permanent discount
    if ($start === null && $end === null) {
        return true;
    }

    if ($start !== null && $now < (int)$start) {
        return false;
    }

    if ($end !== null && $now > (int)$end) {
        return false;
    }
...
function getDiscountedProductPrice($product_price, $product_id)
{
    ...
    if ($product != null && isActiveProductDiscount($product)) {
...
function checkInCart($product_id, $user_id, $limit = 10)
{
    return response()->json([
        'status' => false,
        'message' => 'Product variation support is not available in cart.'
    ]);
}
...
function getDiscountedPrice($data)
{
    ...
    if ($data) {
        foreach ($data as $cart) {
            ...
            // Use the centralized discount validation function
            if (isActiveProductDiscount($cart->product)) {
...
if (!function_exists('variationDiscountedPrice')) {
    ...
    function variationDiscountedPrice($product, $variation, $addTax = false)
    {
        ...
        // Use the centralized discount validation function
        if (isActiveProductDiscount($product)) {
            if ($product->discount_type == 'percent') {
                $discountAmount = ($price * $product->discount_value) / 100;
...
function getDiscountAmount($data)
{
    ...
    if ($data) {
        foreach ($data as $cart) {
            if ($cart->product) {
                $price = ($cart->product_variation->price ?? 0) * $cart->qty;
    ...
    function getSubTotal($carts, $couponDiscount = true, $couponCode = '', $addTax = true)
    {
        ...
        if (count($carts) > 0) {
            foreach ($carts as $cart) {
                ...
                $priceData = variationDiscountedPrice($product, $variation, $addTax);
                $discounted_price = $priceData['discounted_price'];

                $discountedVariationPriceWithTax = $discounted_price;
...
if (!function_exists('isActive')) {
    ...
    if (!function_exists('get_formatted_booking_id')) {
        function get_formatted_booking_id($id)
        {
            ...
            $rawPrefix = ltrim($rawPrefix, '# ');
            
            $prefixLower = strtolower($rawPrefix);

            // Translate prefix keyword only
            if (str_contains($prefixLower, 'booking')) {
                $invPrefix = __('booking.singular_title'); // Booking / BKG
            } elseif (
            ...
            // ✅ FINAL format: "Booking #1234"
            return trim($invPrefix) . ' #' . $id;
        }
    }

    if (!function_exists('get_formatted_order_id')) {
        function get_formatted_order_id($id)
        {
