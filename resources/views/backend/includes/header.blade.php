  ...
  <div class="container-fluid navbar-inner">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="mb-2 navbar-nav ms-auto align-items-center navbar-list mb-lg-0">
        ...
        @if(isset($selected_branch) || (isset($auth_user_branches) && count($auth_user_branches) > 0))
        <li class="nav-item {{ auth()->user()->hasRole('manager') && count($auth_user_branches ?? []) <= 1 ? '' : 'dropdown' }} me-0 me-xl-1 pe-3 border-end iq-dropdown d-flex align-items-center">
          <a href="javascript:void(0)" class="nav-link p-0" @if(!auth()->user()->hasRole('manager') || (isset($auth_user_branches) && count($auth_user_branches) > 1)) data-bs-toggle="dropdown" @endif>
            <div class="iq-sub-card">
              <div class="d-flex align-items-center">
                <span class="iq-media-group">
                  <div class="icon iq-icon-box-3 rounded-pill">{{ isset($selected_branch) ? substr($selected_branch->name, 0, 1) : substr('A',0,1) }}</div>
                </span>
                <div class="ms-3 flex-grow-1">
                  <h6 class="mb-0 ">{{ isset($selected_branch) ? $selected_branch->name : __('branch.all_branches') }}</h6>
                </div>
              </div>
            </div>
          </a>
          ...
          <form action="{{ route('backend.reset-my-work') }}" method="post" class="d-inline ms-2">
            @csrf
            <button type="submit" class="btn btn-secondary btn-sm rounded">
              <span class="btn-inner">{{ __('messages.my_work_active') }}</span>
            </button>
          </form>
          @else
          <form action="{{ route('backend.set-my-work') }}" method="post" class="d-inline ms-2">
            @csrf
            <button type="submit" class="btn btn-primary btn-sm rounded">
              <span class="btn-inner">{{ __('messages.my_work') }}</span>
            </button>
          </form>
          @endif
          @endhasrole

          @if(auth()->user()->hasRole('admin') || (auth()->user()->hasRole('manager') && isset($auth_user_branches) && count($auth_user_branches) > 1))
                ...
                <form action="{{ route('backend.reset-branch') }}" method="post">
                  ...
                  <div class="iq-sub-card {{ !isset($selected_branch) ? 'bg-soft-primary' : '' }} iq-branch-dropdown border-bottom">
                    <div class="d-flex align-items-center">
                      ...
                      <button type="submit" class="btn btn-primary btn-sm rounded">
                        <span class="btn-inner">Apply</span>
                      </button>
                    </div>
                  </div>
                </form>
