...
<head>
    ...
    <script>
        ...
        window.is_manager = @json($is_manager ?? false);
...
<body
    ...
    @php
        ...
        $bannerClasses = '';
        if ($canAccessSidebar) {
            $bannerClasses = !isset($isBanner) ? 'iq-banner default' : 'default';
        }
    @endphp
    <!-- Sidebar -->
    @if ($canAccessSidebar)
    ...
    <div class="main-content wrapper">
        <div class="position-relative pr-hide {{ $bannerClasses }}">
            <!-- Header -->
    ...
    @endif
    <script src="{{ mix('js/core/libs.min.js') }}"></script>
    ...
    <script>
        // Configure Select2 global language settings
        if (typeof $.fn.select2 !== 'undefined') {
            $.fn.select2.defaults.set('language', {
                noResults: function() {
                    return "{{ __('messages.select2_no_results') }}";
                },
                searching: function() {
                    return "{{ __('messages.select2_searching') }}";
                },
                inputTooShort: function(args) {
                    return "{{ __('messages.select2_input_too_short') }}";
                },
                loadingMore: function() {
                    return "{{ __('messages.select2_loading_more') }}";
                }
            });
        }
    </script>
