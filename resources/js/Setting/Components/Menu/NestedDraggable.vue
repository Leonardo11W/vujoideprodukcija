          ...
          <div :class="`btn btn-border flex-column mb-3 w-100 ${element.menu_item_type == 'static' ? 'static-menu-button' : ''}`">
            <div :class="`default-flex-gap justify-content-between w-100 ${element.menu_item_type == 'parent' ? 'mb-3' : ''}`">
              ...
              <div class="default-flex-gap">
                <button class="btn btn-icon btn-secondary btn-sm rounded" v-if="hasPermissions('edit_menu_builder')" :title="$t('messages.edit')" aria-controls="form-modal" @click="openEditModal(element.id)"><i class="fa-solid fa-pen-clip"></i></button>
  ...
  export default {
    ...
    setup() {
      ...
      const openEditModal = (id) => {
        // Set the edit ID in the store
        store.setEditCurrentMenuId(id)
        
        // Get the modal element
        const modalElement = document.getElementById('menuBuilderModal')
        if (modalElement) {
