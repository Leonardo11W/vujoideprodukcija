      ...
      <li v-for="(day, index) in weekdays" class="form-group col-md-12 list-group-item" :key="++index">
        <div class="form-group col-md-12 gap-1">
          ...
          <div class="col-md-12 row row-cols-3">
            <flat-pickr :id="`start_time_${index}`" :name="`start_time_${index}`" class="form-control" v-model="day.start_time" :config="config" :disabled="day.is_holiday ? true : false" :class="{ background_colour: day.is_holiday }"></flat-pickr>
            <flat-pickr :id="`end_time_${index}`" :name="`end_time_${index}`" class="form-control" v-model="day.end_time" :config="config" :disabled="day.is_holiday ? true : false" :class="{ background_colour: day.is_holiday }"></flat-pickr>

            <div class="form-group">
              <div class="d-flex gap-1">
                <div class="form-check">
                  <input class="form-check-input" :value="1" :name="`is_holiday_${index}`" :id="`${index}-dayoff`" type="checkbox" :true-value="1" :false-value="0" v-model="day.is_holiday" />
                </div>
          ...
          <template v-if="!day.is_holiday">
            <div v-for="(input, breakIndex) in day.breaks" :key="breakIndex" class="form-group">
              <h6>{{ $t('setting_business_hours.lbl_break') }}</h6>
              <div class="col-md-12 row row-cols-3">
                <flat-pickr :id="`start_break_${index}_${breakIndex}`" :name="`start_break_${index}_${breakIndex}`" class="form-control" v-model="input.start_break" :config="config"></flat-pickr>
                <flat-pickr :id="`end_break_${index}_${breakIndex}`" :name="`end_break_${index}_${breakIndex}`" class="form-control" v-model="input.end_break" :config="config"></flat-pickr>
                <div>
                  <a class="btn btn-primary" @click="deleteInputField(day, breakIndex)">{{ $t('messages.remove') }}</a>
                </div>
              </div>
            </div>
            <div>
...
const config = ref({
  // Important: allow any minute value (prevents step=5 invalid minute like 32)
  minuteIncrement: 1,
  dateFormat: 'H:i',
  altInput: true,
  altFormat: 'h:i K',
