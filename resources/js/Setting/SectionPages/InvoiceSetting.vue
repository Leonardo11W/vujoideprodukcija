<template>
  <form @submit="formSubmit">
    ...
    <div class="row">
      <InputField class="col-md-6" :is-required="false" :label="$t('setting_invoice.lbl_booking_prefix')" placeholder="# - BKG" v-model="inv_booking_prefix" :error-message="errors.inv_booking_prefix"></InputField>
      <InputField class="col-md-6" type="number" :is-required="false" :label="$t('setting_invoice.lbl_booking_starts')" placeholder="Enter Booking Starts" v-model="booking_code_start" :error-message="errors.booking_code_start"></InputField>
      <InputField class="col-md-6" :is-required="false" :label="$t('setting_invoice.lbl_order_prefix')" placeholder="# - ORD" v-model="inv_prefix" :error-message="errors.inv_prefix"></InputField>
...
const setFormData = (data) => {
  resetForm({
    values: {
      inv_booking_prefix: data.inv_booking_prefix,
      booking_code_start: data.booking_code_start,
...
const validationSchema = yup.object({
  inv_booking_prefix: yup.string().required('Booking Invoice Prefix Is Required'),
  booking_code_start: yup.number().required('Booking Starts Is Required').min(1, 'Booking Starts must be greater than or equal to 1').typeError('Booking Starts must be a valid number'),
...
const { value: inv_booking_prefix } = useField('inv_booking_prefix')
const { value: booking_code_start } = useField('booking_code_start')
...
//fetch data
const data = 'inv_booking_prefix,booking_code_start,inv_prefix,order_code_start,spacial_note,template'
