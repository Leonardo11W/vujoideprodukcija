<template>
  <CardTitle :title="$t('setting_sidebar.lbl_currency_setting')" icon="fa fa-dollar fa-lg mr-2">
    <button class="btn btn-primary" @click="openCurrencyModal(0)"><i class="fas fa-plus-circle me-2"></i> {{$t('messages.new')}}</button>
  </CardTitle>
      ...
      <template v-if="tableList !== null && tableList.length !== 0">
        <tbody>
          <tr v-for="(currency, index) in tableList" :key="index">
            ...
            <th>
              <button type="button" class="btn btn-soft-primary btn-sm me-2" :title="$t('messages.edit')" @click="openCurrencyModal(currency.id)"><i class="fa-solid fa-pen-clip"></i></button>
