<template>
  <CardTitle :title="$t('setting_sidebar.lbl_custom_field')" icon="fa-solid fa-bars">
    <button class="btn btn-primary me-2" v-if="hasPermissions('add_custom_field')" @click="openCustomFieldModal(0)"><i class="fas fa-plus-circle me-2"></i>{{ $t('messages.new') }}</button>
  </CardTitle>
      ...
      <template v-if="tableList !== null && tableList.length !== 0">
        <tbody>
          <tr v-for="(item, index) in tableList" :key="index">
            ...
            <th>
              <button type="button" v-if="hasPermissions('edit_custom_field')" class="btn btn-soft-primary btn-sm" :title="$t('messages.edit')" @click="openCustomFieldModal(item.id)" aria-controls="customFieldModal"><i class="fa-solid fa-pen-clip"></i></button>
