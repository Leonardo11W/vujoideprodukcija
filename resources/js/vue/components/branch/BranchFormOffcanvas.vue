...
const getManagerList = (cb = null, branchId = null) => {
  useSelect({ url: EMPLOYEE_LIST, data: { role: 'manager', branch_id: branchId } }, { value: 'id', label: 'name' }).then((data) => {
      manager.value = data
...
onMounted(() => {
  getServiceList()
  getManagerList(null, currentId.value)
  getCountry()
...
const updateManagerDetail = (e) => {
  switch (e.type) {
    case 'create_manager':
      getManagerList(() => manager_id.value = e.value, currentId.value)
        break;
...
const setFormData = (data) => {
  // Use backend's feature_image URL if present, otherwise use default
  ImageViewer.value = (data.feature_image && typeof data.feature_image === 'string' && data.feature_image.startsWith('http'))
    ? data.feature_image
    : props.defaultImage
  errorMessages.value = {}
  return {
...
const currentId = useModuleId(() => {
  if (currentId.value > 0) {
    getRequest({ url: EDIT_URL, id: currentId.value }).then((res) => {
      if(res.status) {
        getManagerList(() => manager_id.value = res.data.manager_id, currentId.value)
...
const formSubmit = handleSubmit((values) => {
  ...
  // Always stringify address before sending
...
const reset_datatable_close_offcanvas = (res) => {
  ...
  if (res.status) {
    ...
    console.error('Failed to save branch address or image:', res)
  }
}
</script>
