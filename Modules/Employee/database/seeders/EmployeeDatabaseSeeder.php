...
class EmployeeDatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        try {
            $demoLoginEnabled = (int) Setting::where('name', 'is_demo_login')->value('val') === 1;
        } catch (\Throwable $e) {
            $demoLoginEnabled = false;
        }

        // Ensure demo accounts exist & have correct roles even when IS_DUMMY_DATA is off.
        // This fixes cases where staff/manager were created via registration or without roles.
        if (env('IS_DUMMY_DATA') || $demoLoginEnabled) {
            foreach (array_slice($employee, 0, 2) as $employee_data) {
                $featureImage = $employee_data['feature_image'] ?? null;
                $empData = Arr::except($employee_data, ['feature_image']);
                $emp = User::updateOrCreate(['email' => $empData['email']], $empData);
                if (isset($featureImage)) {
                    $this->attachFeatureImage($emp, $featureImage);
                }

                // Fix: Specifically assign Manager Salon to Glamour Cuts (Branch 1)
                $branchId = ($emp->email === 'manager@salon.com') ? 1 : rand(1, 5);
                ...
                EmployeeCommission::create([
                    'employee_id' => $emp->id,
                    'commission_id' => rand(1, 3),
                ]);
                $this->dummyReview($emp->id);

                if ($emp->is_manager) {
                    $emp->syncRoles(['employee', 'manager']);
        ...
        if (env('IS_DUMMY_DATA')) {
            // One manager per branch: branch 1 = Manager Salon (already set above). Branches 2–5 get one manager each.
            $nextBranchForManager = 2;

            foreach (array_slice($employee, 2) as $key => $employee_data) {
                $featureImage = $employee_data['feature_image'] ?? null;
                $empData = Arr::except($employee_data, ['feature_image']);
                $emp = User::updateOrCreate(['email' => $empData['email']], $empData);
                if (isset($featureImage)) {
                ...
                if (!empty($emp->is_manager) && $nextBranchForManager <= 5) {
                    $branchId = $nextBranchForManager;
                    $nextBranchForManager++;
                } else {
                    $branchId = rand(1, 5);
                }
        ...
        if (env('IS_DUMMY_DATA')) {
            // Assign a few services to all employees (including managers)
            $serviceIds = Service::where('status', 1)
                ->orderBy('id')
                ->limit(5)
                ->pluck('id');

            if ($serviceIds->isNotEmpty()) {
                // Managers are also given the 'employee' role in this seeder,
                // so this will include both staff and managers.
        ...
        //         $img = public_path('/dummy-images/staffs/'.rand(1, 20).'.webp');
