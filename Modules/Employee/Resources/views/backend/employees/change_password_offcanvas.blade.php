<form id="employee-change-password-form">
  @csrf
        ...
        <div class="col-12">
          <div class="form-group">
            ...
            <div class="input-group">
              <input type="password" class="form-control" name="old_password" id="cp_old_password" placeholder="{{ __('messages.old_password') }}">
...
(function($){
  ...
  const $form = $('#employee-change-password-form');
  let currentEmployeeId = 0;
  document.addEventListener('employee_assign', (ev) => {
    currentEmployeeId = (ev.detail && ev.detail.form_id) ? parseInt(ev.detail.form_id) : 0;
  })
  ...
  // Reset visibility on close
  offcanvasEl.addEventListener('hidden.bs.offcanvas', function(){
    ['cp_old_password','cp_password','cp_confirm_password'].forEach(function(id){
      const $input = $('#'+id);
  ...
  $form.on('submit', function(e){
    ...
    data.push({name: 'employee_id', value: currentEmployeeId});
    $.post(`{{ url('app/employees/change-password') }}`, $.param(data)).done(function(res){
      if(res?.status){
