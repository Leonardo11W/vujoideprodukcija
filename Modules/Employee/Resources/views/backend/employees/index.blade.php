...
@section('content')
    ...
    @include('employee::backend.employees.form_offcanvas', ['customefield' => $customefield])
    @include('employee::backend.employees.change_password_offcanvas')
    @include('employee::backend.employees.service_offcanvas')
    <div id="employee-page-config"
        data-has-edit="{{ auth()->user()->can('edit_staff') ? 'true' : 'false' }}"
        data-has-delete="{{ auth()->user()->can('delete_staff') ? 'true' : 'false' }}"
        data-has-status="{{ auth()->user()->can('status_staff') ? 'true' : 'false' }}">
    </div>
@endsection
...
@push('after-scripts')
    ...
    <script type="text/javascript" defer>
        ...
        const pageCfg = document.getElementById('employee-page-config')
        ...
        // Can user do ANY bulk action (edit/delete/status)?
        const canShowCheckbox = hasEditPermission || hasDeletePermission || hasStatusPermission

        const canShowActionColumn = {{ auth()->user()->can('edit_staff') || auth()->user()->can('delete_staff') ? 'true' : 'false' }};
