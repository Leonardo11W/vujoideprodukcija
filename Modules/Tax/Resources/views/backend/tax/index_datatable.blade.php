...
<div class="card">
    <div class="card-body">
        <x-backend.section-header>
            ...
            <table id="datatable" class="table border table-responsive" data-can-manage="{{ auth()->user()->can('edit_tax') || auth()->user()->can('delete_tax') ? 1 : 0 }}">
            </table>
        </div>
...
<script type="text/javascript">
    ...
    const canManageTax = datatableEl?.dataset.canManage === '1';
    const finalColumns = [{
            name: 'check',
            data: 'check',
    ...
    const visibleColumns = canManageTax ? finalColumns : finalColumns.filter((column) => column.data !== 'action')
    const defaultOrderIndex = visibleColumns.findIndex((column) => column.data === 'updated_at')
    const defaultOrder = defaultOrderIndex > -1 ? [[defaultOrderIndex, 'desc']] : []
    document.addEventListener('DOMContentLoaded', (event) => {
        initDatatable({
            url: '{{ route("backend.$module_name.index_data") }}',
            finalColumns: visibleColumns,
