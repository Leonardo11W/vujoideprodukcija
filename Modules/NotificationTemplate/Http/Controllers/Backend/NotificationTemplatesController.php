    ...
    public function index_data(Datatables $datatable)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ->addColumn('check', function ($row) {
                $user = auth()->user();
                $hasActionPermission = $user->can('edit_notification_template') || $user->can('delete_notification_template');
                if (!$hasActionPermission) {
                    return '';
                }
                if (!$row->status && !$user->can('edit_notification_template')) {
                    return '';
                }
            ...
            ->editColumn('status', function ($row) {
                $canChangeStatus = auth()->user()->hasRole('admin') || auth()->user()->can('edit_notification_template');
                $checked = $row->status ? 'checked="checked"' : '';
                $disabled = $canChangeStatus ? '' : 'disabled';
                return '
                    <div class="form-check form-switch ">
                        <input type="checkbox" data-url="' . route('backend.notificationtemplates.update_status', $row->id) . '" data-token="' . csrf_token() . '" class="switch-status-change form-check-input"  id="datatable-row-' . $row->id . '"  name="status" value="' . $row->id . '" ' . $checked . ' ' . $disabled . '>
                    </div>
                ';
            })
