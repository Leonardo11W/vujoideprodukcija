...
class PackagesController extends Controller
{
    ...
    public function index_data(Request $request)
    {
        ...
        return Datatables::of($query)
            ->addColumn("check", function ($data) {
                $user = auth()->user();
                $hasActionPermission = $user->can("edit_package") || $user->can("delete_package");
                if (!$hasActionPermission) {
                    return "";
                }
                if (!$data->status && !$user->can("edit_package")) {
                    return "";
                }
            ...
            ->editColumn('status', function ($data) {
                $canChangeStatus = auth()->user()->hasRole('admin') || auth()->user()->can('edit_package');
                $checked = $data->status ? 'checked="checked"' : '';
                $disabled = $canChangeStatus ? '' : 'disabled';
                return '
                    <div class="form-check form-switch  ">
                        <input type="checkbox" data-url="' . route('backend.package.update_status', $data->id) . '" data-token="' . csrf_token() . '" class="switch-status-change form-check-input"  id="datatable-row-' . $data->id . '"  name="status" value="' . $data->id . '" ' . $checked . ' ' . $disabled . '>
                    </div>
                ';
            })
            // package_price
            ->addColumn('qty', function ($data) {
                $canEdit = auth()->user()->hasRole('admin') || auth()->user()->can('edit_package');
                $disabled = $canEdit ? '' : 'disabled';
                $count = isset($data->service) ? $data->service->count() : 0;
                return "
        <button type='button' data-custom-module='{$data->id}' data-assign-module='{$data->id}' data-assign-target='#package-service-form' data-custom-event='custom_form' data-assign-event='package_service_form' class='btn btn-primary btn-sm rounded' {$disabled}>{$count}</button>";
            })
            ...
            ->orderColumn('name', function ($query, $order) {
                $query->orderBy('packages.name', $order);
            })
            ->orderColumn('branch', function ($query, $order) {
                $query->select('packages.*')
            ...
            ->orderColumn('status', function ($query, $order) {
                $query->orderBy('packages.status', $order);
            })
    ...
    public function services_index_list(Request $request)
    {
        ...
        \Log::info('package.services_index_list called', [
        ...
        \Log::info('package.services_index_list: result count', ['branch_id' => $branch_id, 'employee_id' => $employee_id, 'count' => $data->count()]);
    ...
    {
        // Decode JSON strings or comma-separated strings into arrays
        $request['services'] = is_string($request->services) && !empty($request->services) ? json_decode($request->services, true) : [];
        ...
        foreach ($request['services'] as $serviceItem) {
            ...
            $servicesToCreate[] = $filteredService;
        }
        ...
        if (!empty($servicesToCreate)) {
            $data->services()->createMany($servicesToCreate);
        }
    ...
    {
        // Correctly decode services JSON when updating
        $request['services'] = is_string($request->services) && !empty($request->services) ? json_decode($request->services, true) : [];
        $request['employee_id'] = is_string($request->employee_id) && !empty($request->employee_id) ? explode(',', $request->employee_id) : [];
        $request['category_id'] = is_string($request->category_id) && !empty($request->category_id) ? explode(',', $request->category_id) : [];
