...
const removeService = (index) => {
  const removedId = selectedServices.value[index]?.service_id
  if (removedId !== undefined && removedId !== '') {
    const idPos = service_id.value.indexOf(removedId)
    if (idPos > -1) service_id.value.splice(idPos, 1)
    const namePos = service_name.value.indexOf(removedId)
    if (namePos > -1) service_name.value.splice(namePos, 1)
  }
  ...
  // Remove any previous selection for this row from tracking arrays
  const previousId = selectedServices.value[index]._previous_service_id
  if (previousId && previousId !== serviceId) {
    const prevIdPos = service_id.value.indexOf(previousId)
    if (prevIdPos > -1) service_id.value.splice(prevIdPos, 1)
    const prevNamePos = service_name.value.indexOf(previousId)
    if (prevNamePos > -1) service_name.value.splice(prevNamePos, 1)
  }
  // Track current selection
  if (!service_id.value.includes(serviceId)) service_id.value.push(serviceId)
  if (!service_name.value.includes(serviceId)) service_name.value.push(serviceId)
  selectedServices.value[index]._previous_service_id = serviceId
...
const setFormData = (data) => {
  console.log(data)
  ImageViewer.value = data && data.package_image ? data.package_image : null;
  selectedServices.value = Array.isArray(data && data.service) ? data.service : []
  // Sync selected IDs/names for filters and tracking
  service_id.value = (selectedServices.value || []).map(s => s.service_id)
  service_name.value = (selectedServices.value || []).map(s => s.service_id)
  resetForm({
    values: {
      name: data.name,
...
const formSubmit = handleSubmit((values) => {
  ...
  // Also include raw selection array for backend if required
  values.service = JSON.stringify(selectedServices.value)
  if (currentId.value > 0) {
