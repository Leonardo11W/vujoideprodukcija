...
@include('customer::backend.customers.form_offcanvas', ['customefield' => $customefield])
@include('customer::backend.customers.change_password_offcanvas')
@endsection
...
<script type="text/javascript" defer>
  const columns = [{
    ...
    ;var customFieldColumns = (function(){
      try {
        var parsed = JSON.parse(String.raw`@json($columns)`)
        return Array.isArray(parsed) ? parsed : []
      } catch(e) { return [] }
    })()
    const canShowActionColumn = {{ auth()->user()->can('edit_customer') || auth()->user()->can('delete_customer') ? 'true' : 'false' }};
    var finalColumns = [
      ...columns,
      ...customFieldColumns,
