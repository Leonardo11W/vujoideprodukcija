        ...
        <div class="card-body">
          <x-backend.section-header>
            <div class="d-flex flex-wrap gap-3">
              @canany(['edit_product_category', 'delete_product_category'])
...
@push('after-scripts')
    ...
    const columns = [
            ...
            { data: 'brand_id', name: 'brand_id', title: "{{ __('category.lbl_brand') }}", width: '15%', render: function(data, type, row, meta) {
                if (!data) return '';
                try {
                  var html = String(data).replace(/<br\s*\/?\>/gi, ',');
                  var tmp = document.createElement('div');
                  tmp.innerHTML = html;
                  var text = (tmp.textContent || tmp.innerText || '').trim();
                  var parts = text.split(',').map(function(s){ return s.trim(); }).filter(Boolean);
                  if (parts.length <= 3) {
                    return parts.map(function(p){ return '<div>' + escapeHtml(p) + '</div>'; }).join('');
                  }
                  var first = parts.slice(0,3).map(function(p){ return '<div>' + escapeHtml(p) + '</div>'; }).join('');
                  var rest = parts.slice(3).map(function(p){ return '<div>' + escapeHtml(p) + '</div>'; }).join('');
                  var id = 'brand-more-' + Math.random().toString(36).substr(2,9);
                  return '<div>' + first + '<span id="' + id + '" class="d-none">' + rest + '</span> <a href="#" class="brand-show-more" data-target="#' + id + '">Show more (' + (parts.length - 3) + ')</a></div>';
                } catch (e) {
                  return data;
                }
              }
        ...
        const canShowActionColumn = {{ auth()->user()->can('edit_product_category') || auth()->user()->can('delete_product_category') ? 'true' : 'false' }};
        ...
        $(document).on('click', '.brand-show-more', function(e){
          e.preventDefault();
          var $link = $(this);
