...
class OrdersController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        ...
        if (count($carts) > 0) {
            ...
            foreach ($carts as $cart) {
                ...
                $tax_data = getTaxamount($priceData['discounted_price']);
            ...
            // Load the orderGroup relationship to include order_code
    ...
    {
        $order_status = Constant::where('type', 'ORDER_STATUS')
            ->where('name', '!=', 'pending')
            ->get();

        $order_status->transform(function ($item) {
            $item->name = str_replace('order_', '', $item->name);
            return $item;
        });
    ...
    public function orderList(Request $request)
    {
        ...
        if ($request->has('delivery_status') && $request->delivery_status != '') {
            ...
            \Illuminate\Support\Facades\Log::info('OrderList Debug', [
                'original_input' => $request->delivery_status,
                'processed_status' => $delivery_status,
                'user_id' => $userId
            ]);
        ...
        if ($request->filled('search')) {
           ...
           $orderQuery->whereHas('orderItems.product_variation.product', function ($q) use ($search) {
    ...
    public function orderDetails(Request $request)
    {
        ...
        $orderQuery = Order::where('id', $orderId)->with('orderItems.product', 'orderGroup', 'orderUpdates')->first();
