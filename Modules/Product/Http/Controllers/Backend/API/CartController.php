...
class CartController extends Controller
{
    ...
    public function store(CartRequest $request)
    {
        ...
        if ($existingCartItem) {
            // Add to existing quantity in cart
            $newQty = $existingCartItem->qty + $data['qty'];
            
            // Check stock availability
            $product_variation = ProductVariation::where('id', $data['product_variation_id'])->first();
            if ($product_variation) {
                $get_total_stock = $product_variation->product_variation_stock->stock_qty;
                
                if ($newQty > $get_total_stock) {
            ...
            $existingCartItem->update(['qty' => $newQty]);
            $cartCollection = new CartResource($existingCartItem->fresh());
            $message = __('product.cart_quantity_updated');
