...
class ProductsController extends Controller
{
    ...
    public function index_data(Request $request, Datatables $datatable)
    {
        ...
        // If logged-in user is a manager, restrict products to manager's branches
        ...
        //         $selectedBranchId = $request->input('selected_session_branch_id') ?: session('selected_branch_id') ?: null;
        //         if ($selectedBranchId && in_array($selectedBranchId, $managerBranchIds)) {
        //             // Manager selected a branch: show only products for that branch
        //             $query->where('branch_id', $selectedBranchId);
        //         } elseif (count($managerBranchIds) === 1) {
        //             // Manager has a single branch: show only products for that branch
        //             $query->where('branch_id', $managerBranchIds[0]);
        //         } else {
        //             // Manager with multiple branches: show products for any of their branches
        ...
        ->addColumn('check', function ($row) {
            ...
            if (!$row->status && !$user->can('edit_product')) {
                return '';
            }
            ...
            ->editColumn('name', function ($data) {
                return view('product::backend.products.product_id', compact('data'));
            })
            ->editColumn('is_featured', function ($data) {
                $canChangeFeatured = auth()->user()->hasRole('admin') || auth()->user()->can('edit_product');
                $checked = $data->is_featured ? 'checked="checked"' : '';
                $disabled = $canChangeFeatured ? '' : 'disabled';

                return '
                            <div class="form-check form-switch ">
                                <input type="checkbox" data-url="' . route('backend.products.update_is_featured', $data->id) . '" data-token="' . csrf_token() . '" class="switch-status-change form-check-input"  id="datatable-row-' . $data->id . '"  name="is_featured" value="' . $data->id . '" ' . $checked . ' ' . $disabled . '>
                            </div>
                          ';
            })
            ->editColumn('status', function ($row) {
                $canChangeStatus = auth()->user()->hasRole('admin') || auth()->user()->can('edit_product');
                ...
                return '
                    <div class="form-check form-switch d-flex justify-content-center">
                        <input 
                            ...
                            data-url="' . route('backend.products.update_status', $row->id) . '"
    ...
    public function store(Request $request)
    {
        ...
        // Allow branch_id from request (e.g., admin selection)
        if ($request->has('branch_id') && ! is_null($request->branch_id) && $request->branch_id !== '') {
            $product->branch_id = $request->branch_id;
        }

        // Auto-assign branch for manager users
        if (auth()->check() && auth()->user()->hasRole('manager')) {
            $managerBranchIds = auth()->user()->branches()->pluck('branches.id')->toArray();
            $primaryBranchIds = Branch::where('manager_id', auth()->id())->pluck('id')->toArray();
            $managerBranchIds = array_values(array_unique(array_merge($managerBranchIds, $primaryBranchIds)));
            ...
            if (! empty($managerBranchIds)) {
                ...
                if ($selectedBranchId && in_array($selectedBranchId, $managerBranchIds)) {
                    ...
                    $product->branch_id = $managerBranchIds[0];
                }
            }
        }
        ...
        if (! $location) {
            return response()->json(['message' => 'Please configure at least one stock location before saving products.', 'status' => false], 422);
        }
        if (! $location) {
            $location = Location::first();
        }
        ...
        // if ($request->has('feature_image_count') && $request->feature_image_count > 0) {
        //     for ($i = 0; $i < $request->feature_image_count; $i++) {
        //         $fileKey = 'feature_image_' . $i;
        //         if ($request->hasFile($fileKey) && $request->file($fileKey)->isValid()) {
        //             storeMediaFile($product, $request->file($fileKey), 'feature_image');
        //         }
        //     }
        // }
        ...
        if ($request->has('feature_image_count') && $request->feature_image_count > 0) {
            ...
            if (!empty($files)) {
                storeMultipleMediaFiles($product, $files, 'feature_images', true);
            }
        }
    ...
    public function edit($id)
    {
        ...
        if ($data->has_variation) {
            ...
            foreach ($combinations as $key => $value) {
                $stock = optional($value->product_variation_stock()->where('location_id', optional($location)->id)->first())->stock_qty ?? 0;
                $combinations[$key] = [
            ...
            $stock = optional(optional($variation)->product_variation_stock()->where('location_id', optional($location)->id)->first())->stock_qty ?? 0;
            $data['stock'] = $stock;
        ...
        $featureImages = [];
        $mediaItems = $data->getMedia('feature_images');
        ...
        $data['feature_image_count'] = count($featureImages);
    ...
    public function update(ProductRequest $request, $id)
    {
        ...
        // Auto-assign branch for manager users on update
        if (auth()->check() && auth()->user()->hasRole('manager')) {
            $managerBranchIds = auth()->user()->branches()->pluck('branches.id')->toArray();
            $primaryBranchIds = Branch::where('manager_id', auth()->id())->pluck('id')->toArray();
            $managerBranchIds = array_values(array_unique(array_merge($managerBranchIds, $primaryBranchIds)));
        ...
        if ($request->has('existing_feature_image_ids')) {
            $existingImageIds = json_decode($request->existing_feature_image_ids, true);
        }

        // Delete any removed images
        $mediaItems = $product->getMedia('feature_images');
        foreach ($mediaItems as $media) {
            if (! in_array($media->id, $existingImageIds)) {
                $media->delete();
            }
        }
        ...
        if ($request->has('feature_image_count') && $request->feature_image_count > 0) {
            ...
            if (!empty($files)) {
                storeMultipleMediaFiles($product, $files, 'feature_images', false);
            }
        }

        // Optional: Clear all if no image is to be kept
        if ((int) $request->feature_image_count === 0) {
            $product->clearMediaCollection('feature_images');
        }
