...
export const SERVICE_LIST = ({ employee_id, branch_id }) => {
  const params = []
  if (branch_id !== undefined && branch_id !== null && String(branch_id) !== '') params.push(`branch_id=${branch_id}`)
  if (employee_id !== undefined && employee_id !== null && String(employee_id) !== '' && String(employee_id) !== 'undefined') params.push(`employee_id=${employee_id}`)
  const qs = params.length ? `?${params.join('&')}` : ''
  return { path: `${MODULE}/services-index_list${qs}`, method: 'GET' }
}

export const SLOT_LIST = ({ date, employee_id, branch_id }) => {
  const params = []
  if (date !== undefined && date !== null && String(date) !== '') params.push(`date=${date}`)
  if (branch_id !== undefined && branch_id !== null && String(branch_id) !== '') params.push(`branch_id=${branch_id}`)
  if (employee_id !== undefined && employee_id !== null && String(employee_id) !== '' && String(employee_id) !== 'undefined') params.push(`employee_id=${employee_id}`)
  const qs = params.length ? `?${params.join('&')}` : ''
  return { path: `${MODULE}/slots${qs}`, method: 'GET' }
}
