  ...
  <div class="offcanvas-body">
    ...
    <div class="form-group d-flex align-items-center justify-content-between" v-for="(tax, index) in taxes" :key="index">
      ...
      <template v-else>
        ...
        <strong>+ {{ formatCurrencyVue(tax.amount ?? tax.tax_amount ?? 0) }}</strong>
      </template>
    </div>
...
const formatCurrencyVue = (value) => {
  if (value === undefined || value === null || isNaN(value)) return window.currencyFormat ? window.currencyFormat(0) : 0
  if (window.currencyFormat !== undefined) {
    return window.currencyFormat(Number(value))
  }
...
const taxAmount = computed(() => {
  ...
  for (const tax of data.taxes) {
    if (tax.type === 'percent') {
      totalTaxAmount += ((Number(finalsubtotal.value) || 0) * (Number(tax.percent) || 0)) / 100
    } else {
      totalTaxAmount += Number(tax.amount ?? tax.tax_amount ?? 0)
    }
  }
