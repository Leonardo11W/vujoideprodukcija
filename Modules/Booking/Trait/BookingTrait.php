...
trait BookingTrait
{
    public function updateBookingService($data, $booking_id)
    {
        ...
        if ($serviceData->isEmpty()) {
            BookingService::where('booking_id', $booking_id)->delete();
            return;
        }
        ...
        $serviceIds = $serviceData->pluck('service_id')->filter()->unique()->toArray();

        $branchServiceLookup = collect();
        if (!empty($serviceIds) && $branchId) {
            $branchServiceLookup = ServiceBranches::where('branch_id', $branchId)
                ->whereIn('service_id', $serviceIds)
                ->get()
                ->keyBy('service_id');
        }

        $serviceDefaults = !empty($serviceIds)
            ? Service::whereIn('id', $serviceIds)
                ->get(['id', 'default_price', 'duration_min'])
                ->keyBy('id')
            : collect();
        ...
        foreach ($serviceData as $key => $value) {
            ...
            $branchService = $branchServiceLookup->get($serviceId);
            $service = $serviceDefaults->get($serviceId);
            ...
            } elseif (isset($value['service_price']) && $value['service_price'] > 0) {
                $servicePrice = $value['service_price'];
            } elseif ($service && isset($service->default_price)) {
                $servicePrice = $service->default_price;
            } else {
                $servicePrice = 0;
            }

            // Use branch-specific duration if available and greater than 0
            $duration = null;
            if ($branchService && isset($branchService->duration_min) && $branchService->duration_min > 0) {
                $duration = $branchService->duration_min;
            } elseif (isset($value['duration_min']) && $value['duration_min'] > 0) {
                $duration = $value['duration_min'];
            } elseif ($service && isset($service->duration_min)) {
                $duration = $service->duration_min;
            } else {
                $duration = 30;
            }

            BookingService::updateOrCreate(['booking_id' => $booking_id, 'service_id' => $serviceId, 'employee_id' => $value['employee_id']], [
                'sequance' => $key,
    ...
    public function getSlots($date, $day, $branch_id, $serviceDuration=0, $employee_id = null)
    {
        ...
        if (isset($slotDay)) {
            \Log::info('Breaks for slot generation', ['breaks' => $slotDay->breaks, 'branch_id' => $branch_id, 'day' => $day, 'date' => $date]);
    ...
    protected function sendNotificationOnBookingUpdate($type, $notify_message, $booking, $notify = true)
    {
        ...
        $bookingDurationMin = (int) ($booking->services?->sum('duration_min') ?? 0);
        $checkOutDateTime = !empty($booking->end_date_time)
            ? $booking->end_date_time
            : date('Y-m-d H:i:s', strtotime($booking->start_date_time) + ($bookingDurationMin * 60));
        ...
        $assignedEmployee = optional($booking->services->first())->employee;
        ...
        $data['booking'] = [
            ...
            // Use the assigned staff for notifications instead of branch manager
