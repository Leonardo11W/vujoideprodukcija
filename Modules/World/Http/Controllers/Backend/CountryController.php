...
class CountryController extends Controller
{
    ...
    public function __construct()
    {
        ...
        // Backend listing screens still require `view_location`, but the
        // lightweight select / API endpoints (index_list) are now public,
        // so mobile apps and external clients can load countries without
        // needing panel permissions.
        $this->middleware('permission:view_location', ['only' => ['index', 'show', 'index_data']]);
        $this->middleware('permission:add_location', ['only' => ['create', 'store']]);
        $this->middleware('permission:edit_location', ['only' => ['edit', 'update', 'update_status']]);
        $this->middleware('permission:delete_location', ['only' =>['destroy', 'bulk_action']]);
    }
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('status', function ($data) {
                ...
                $disabled = auth()->user()->can('edit_location') ? '' : 'disabled';

                return '
                                <div class="form-check form-switch ">
                                    <input type="checkbox" data-url="' . route('backend.country.update_status', $data->id) . '" data-token="' . csrf_token() . '" class="switch-status-change form-check-input"  id="datatable-row-' . $data->id . '"  name="status" value="' . $data->id . '" ' . $checked . ' ' . $disabled . '>
                                </div>
                            ';
            })
