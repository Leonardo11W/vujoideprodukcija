...
class PromotionsController extends Controller
{
    ...
    {
        if(!auth()->user()->can('promotion_coupon')) {
            return redirect()->route('backend.dashboard')->with('errors', 'You do not have permission to access this page.');
        }
        $filter = [
    ...
    {
        if(!auth()->user()->can('view_promotion')) {
            if ($request->expectsJson() || $request->ajax()) {
                return response()->json(['message' => 'You do not have permission to access this page.', 'status' => false], 403);
            }
            return redirect()->route('backend.dashboard')->with('errors', 'You do not have permission to access this page.');
        }
        $term = trim($request->q);
    ...
    public function bulk_action(Request $request)
    {
        ...
        if ($actionType == 'change-status' && !auth()->user()->can('edit_promotion')) {
            return response()->json(['message' => 'You do not have permission to perform this action.', 'status' => false], 403);
        }
        if ($actionType == 'delete' && !auth()->user()->can('delete_promotion')) {
            return response()->json(['message' => 'You do not have permission to perform this action.', 'status' => false], 403);
        }
    ...
    {
        if(!auth()->user()->can('edit_promotion')) {
            return response()->json(['message' => 'You do not have permission to perform this action.', 'status' => false], 403);
        }
    ...
    {
        if(!auth()->user()->can('add_promotion')) {
            return response()->json(['message' => 'You do not have permission to perform this action.', 'status' => false], 403);
        }
    ...
    {
        if(!auth()->user()->can('delete_promotion')) {
            return response()->json(['message' => 'You do not have permission to perform this action.', 'status' => false], 403);
        }
        if (env('IS_DEMO')) {
            return response()->json(['message' => __('messages.permission_denied'), 'status' => false], 200);
