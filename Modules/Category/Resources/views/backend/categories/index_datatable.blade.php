  ...
  <div class="card-body">
    <x-backend.section-header>
      <div class="d-flex flex-wrap gap-3">
        @if(auth()->user()->can('edit_service_category') || auth()->user()->can('delete_service_category'))
      ...
      <x-slot name="toolbar">
        ...
        @hasPermission('add_service_category')
    ...
    <div id="category-page-config"
        data-has-edit="{{ auth()->user()->can('edit_service_category') ? 'true' : 'false' }}"
        data-has-delete="{{ auth()->user()->can('delete_service_category') ? 'true' : 'false' }}"
        data-has-status="{{ auth()->user()->can('status_category') ? 'true' : 'false' }}">
    </div>
@include('category::backend.categories.form_offcanvas', [
'defaultImage' => default_feature_image(),
'createTitle' => __('messages.new') . ' ' . __('category.singular_title'),
'editTitle' => __('messages.edit') . ' ' . __('category.singular_title'),
'isSubCategory' => false,
])
@endsection
...
<script type="text/javascript" defer>
  ...
  const pageCfg = document.getElementById('category-page-config');
  ...
  const canShowActionColumn = hasEditPermission || hasDeletePermission;
