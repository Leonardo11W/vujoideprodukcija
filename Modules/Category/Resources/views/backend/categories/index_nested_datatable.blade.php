  ...
  <div class="card-body">
    <x-backend.section-header>
      <div class="d-flex flex-wrap gap-3">
        @if(auth()->user()->can('edit_service_subcategory') || auth()->user()->can('delete_service_subcategory') || auth()->user()->can('add_service_subcategory'))
        <x-backend.quick-action url='{{route("backend.categories.bulk_action")}}'>
          <div class="">
            <select name="action_type" class="form-control select2 col-12" id="quick-action-type" style="width:100%">
              <option value="">{{ __('messages.no_action') }}</option>
              @can('edit_service_subcategory')
              <option value="change-status">{{ __('messages.status') }}</option>
              @endcan
              @can('delete_service_subcategory')
      ...
      <x-slot name="toolbar">
        ...
        @hasPermission('add_service_subcategory')
    ...
    <div id="subcategory-page-config"
        data-has-edit="{{ auth()->user()->can('edit_service_subcategory') ? 'true' : 'false' }}"
        data-has-delete="{{ auth()->user()->can('delete_service_subcategory') ? 'true' : 'false' }}"
        data-has-status="{{ auth()->user()->can('status_subcategory') ? 'true' : 'false' }}">
    </div>
@include('category::backend.categories.form_offcanvas', [
'defaultImage' => default_feature_image(),
'createTitle' => __('messages.new') . ' ' . __('category.sub_category'),
'editTitle' => __('messages.edit') . ' ' . __('category.sub_category'),
'isSubCategory' => true,
'parentDefault' => $parentID ?? null,
])
@endsection
...
<script type="text/javascript">
  ...
  const pageCfg = document.getElementById('subcategory-page-config')
  ...
  const canShowActionColumn = {{ auth()->user()->can('edit_service_subcategory') || auth()->user()->can('delete_service_subcategory') ? 'true' : 'false' }};
...
finalColumns = [
    ...finalColumns,
    ...columns.filter(col => col.name !== 'check'),
    ...customFieldColumns,
