    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ->addColumn('check', function ($row) {
                $user = auth()->user();
                $hasActionPermission = $user->can('edit_app_banner') || $user->can('delete_app_banner');
                if (!$hasActionPermission) {
                    return '';
                }
                if (!$row->status && !$user->can('edit_app_banner')) {
                    return '';
                }
            ...
            ->addColumn('action', function ($data) use ($module_name) {
             ...
             ->editColumn('name', function ($data) {
                return view('slider::backend.sliders.slider_id', compact('data'));
            })
            ...
            ->editColumn('status', function ($row) {
                $canChangeStatus = auth()->user()->hasRole('admin') || auth()->user()->can('edit_app_banner');
                $checked = $row->status ? 'checked="checked"' : '';
                $disabled = $canChangeStatus ? '' : 'disabled';

                return '
                    <div class="form-check form-switch ">
                        <input type="checkbox" data-url="'.route('backend.app_banners.update_status', $row->id).'" data-token="'.csrf_token().'" class="switch-status-change form-check-input"  id="datatable-row-'.$row->id.'"  name="status" value="'.$row->id.'" '.$checked.' '.$disabled.'>
                    </div>
                ';
            })
...
dd($data);
    ...
    public function uniqueSliders(Request $request)
    {
        $sliderName = $request->input('name'); // The name field is 'name', not 'service'
        $sliderId = $request->input('id'); // The unique check will be based on 'id'

        $isUnique = true;
        if (!$sliderId) {
            $isUnique = Slider::where('name', $sliderName)
                ->doesntExist();
        }
