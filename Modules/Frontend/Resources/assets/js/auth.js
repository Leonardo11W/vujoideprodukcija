// Mobile input validation with IntlTelInput
...
function validateRegisterForm() {
  ...
  if (!password.value.trim()) {
    ...
    showValidationError(password, 'Password must be between 8 to 14 characters.');
    isValid = false;
  } else if (password.value.length > 14) {

    showValidationError(password, 'Password must not exceed 14 characters.');
    isValid = false;
  } else {
    clearValidationError(password);
  }

  if (password.value.length >= 8 && password.value.length <= 14 && password.value !== confirmPassword.value) {
    ...
    if (!mobile.value.trim()) {
      ...
      if (window.iti && typeof window.iti.isValidNumber === 'function') {
        if (!window.iti.isValidNumber()) {
          showValidationError(mobile, 'Please enter a valid phone number.');
          isValid = false;
        } else {
