        ...
        try {
            ...
            return $datatable->eloquent($query)
                ->addColumn('check', function ($query) {
                    $user = auth()->user();
                    $hasActionPermission = $user->can('blog edit') || $user->can('blog delete');
                    if (!$hasActionPermission) {
                        return '';
                    }
                    if (!$query->status && !$user->can('blog edit')) {
                        return '';
                    }
                ...
                ->editColumn('status', function ($row) {
                    $canChangeStatus = auth()->user()->hasRole('admin') || auth()->user()->can('blog edit');
                    $checked = $row->status ? 'checked="checked"' : '';
                    $disabled = $canChangeStatus ? '' : 'disabled';
                    return '
                        <div class="form-check form-switch ">
                            <input type="checkbox" data-url="'.route('backend.blog.update_status', $row->id).'" data-token="'.csrf_token().'" class="switch-status-change form-check-input"  id="datatable-row-'.$row->id.'"  name="status" value="'.$row->id.'" '.$checked.' '.$disabled.'>
                        </div>
                    ';
                })
